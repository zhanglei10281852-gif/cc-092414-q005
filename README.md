...
- 追溯事件链：以哈希链记录来源、拆分、合并与节点交接，强制数量平衡与时间顺序，支持撤销标记（不删除历史）和上下游双向溯源；事件表由数据库触发器禁止修改与删除。
...
测试覆盖身份初始化、登录、用户与角色维护、权限计算、账号停用后的会话撤销、审计脱敏、农产品批次、抽样检测、运输追踪、风险任务、追溯事件链（拆分/合并/交接/数量平衡/逆序拒绝/撤销/双向溯源/防篡改）和数据库时间格式。
...
app/
  ...
  trace/            不可篡改的追溯事件链（哈希链、拆分合并、交接、撤销与双向溯源）
  schemas/         管理接口输入模型
